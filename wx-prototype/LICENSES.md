...
| **NOAA/NWS GEFS** (NOMADS grib filter + NODD) | 0.5° `pgrb2ap5`: `geavg` μέσος όρος + `gespr` διασπορά (από 30 διαταραγμένα μέλη) — θερμοκρασία, άνεμος, υετός, νέφωση. 0.25° `pgrb2sp25`: ριπές (GUST), που δεν υπάρχουν στο 0.5° | US Public Domain (17 USC §105) | ✅ | ✅ | Ίδιο καθεστώς με GFS (NWS disclaimer: «may be used without charge for any lawful purpose»). Απαγορεύεται η υποδήλωση σχέσης/έγκρισης NOAA και η παρουσίαση τροποποιημένου υλικού ως επίσημου. Χρήση: panel «Σύνολο GEFS» (μέσος όρος/διασπορά, PRO). Αναφορά NOAA στο UI |
...
**Κατάσταση: λυμένο (δεν υπάρχει χάρτης).** Το UI δεν φορτώνει βιβλιοθήκη χάρτη
και δεν ζητά raster tiles, οπότε δεν υπάρχει πάροχος προς αδειοδότηση. Η θέση
επιλέγεται με αναζήτηση κειμένου (Photon), geolocation, αγαπημένα ή χειροκίνητες
συντεταγμένες.

Ιστορική σημείωση: κάποτε υπήρχαν `WX_TILE_URL`/`WX_TILE_SUBDOMAINS`/`WX_TILE_RETINA`/
`WX_TILE_COMMERCIAL_OK`. **Δεν διαβάζονταν ποτέ από τον κώδικα** και έχουν
αφαιρεθεί από το `.env.example`/README, ώστε να μην υπόσχεται λειτουργία που δεν
υπάρχει. Η παρακάτω έρευνα παρόχων διατηρείται για την περίπτωση που προστεθεί
χάρτης στο μέλλον.
...
### Σημείωση για MPL-2.0 (notifications)

Το `pywebpush` και το `py-vapid` είναι **MPL-2.0**: copyleft σε επίπεδο αρχείου.
Αυτό σημαίνει ότι αν τροποποιήσεις αρχείο *αυτών των βιβλιοθηκών*, οι αλλαγές
σε εκείνο το αρχείο πρέπει να δημοσιευτούν με MPL-2.0. Χρήση ως εξαρτήσεις από
δικό μας κώδικα (ό,τι κάνει το project) δεν επιβάλλει MPL-2.0 στον δικό μας
κώδικα, οπότε η εμπορική διάθεση επιτρέπεται. Δεν τροποποιούνται αρχεία τους
εδώ — καλούνται μόνο.
...
- [x] **Πλακίδια χάρτη** — δεν χρησιμοποιούνται. Το UI δεν έχει χάρτη, οπότε δεν
      υπάρχει πάροχος προς αδειοδότηση (αποκλεισμός #2).
