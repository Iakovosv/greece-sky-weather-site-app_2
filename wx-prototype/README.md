...
### Διασπορά GEFS ensemble

Η απόκλιση των τριών παραπάνω μοντέλων είναι σήμα από τρία σημεία. Το `ensemble.py`
προσθέτει ένα **distribution-aware** μέτρο δίπλα της: το πραγματικό ensemble του
GEFS, από τα προϋπολογισμένα πεδία `geavg` (μέσος όρος) και `gespr` (διασπορά).
Το NOMADS grib filter τα περιορίζει server-side στο ελληνικό πλαίσιο, οπότε ένα
βήμα κοστίζει λίγα KB αντί ~470 MB για τα 30 αρχεία μελών.

**Τι είναι ακριβώς** (επαληθευμένο από την τεκμηρίωση NCEP και τα GRIB metadata,
όχι υπόθεση):

- Το `gespr` είναι η διασπορά του ensemble, ορισμού «παρόμοια με τυπική απόκλιση»
  γύρω από τον μέσο όρο, στο ίδιο unit με το πεδίο.
...
Είναι **προσθετικό και best-effort**: αν το NOMADS δεν απαντήσει, η κάρτα πέφτει
στην απόκλιση των τριών μοντέλων, όπως σήμερα. Είναι PRO-only (ζει στο `expert`
block του `/api/brief`)· ο FREE δεν το κατεβάζει και δεν το cache-άρει καθόλου.
Το cache key είναι provider/kind/run/step/variable — ποτέ συντεταγμένες — ώστε
όλοι οι PRO χρήστες μιας περιοχής να μοιράζονται ένα download ανά κύκλο, με
single-flight για ταυτόχρονα cold requests και stale fallback μόνο όταν το φρέσκο
fetch έχει ήδη αποτύχει.
