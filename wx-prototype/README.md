...
(CC BY 4.0), GEFS ensemble mean/spread (NOAA, public domain) και Photon/OSM για geocoding. Ο κατάλογος αδειών και οι εκκρεμότητες
...
- **PRO** — 240 ώρες (10 ημέρες) συν Skew-T, δείκτες αστάθειας (SBCAPE, shear,
  SRH, LCL), σύγκριση μοντέλων με απόκλιση ως ένδειξη συμφωνίας και
  κατακόρυφη δομή.
...
Το PRO δείχνει GFS, ICON-EU και ECMWF δίπλα-δίπλα και υπολογίζει την **απόκλιση
GFS και ICON-EU** ανά ώρα (μέγιστη μείον ελάχιστη). Το αποτέλεσμα χαρακτηρίζεται
«υψηλή», «μέτρια» ή «χαμηλή» συμφωνία, με τους αριθμούς (μέση/μέγιστη απόκλιση)
ορατούς.
...
### Σύνολο GEFS (ensemble mean/spread)

Η απόκλιση των δύο παραπάνω μοντέλων είναι σήμα από δύο σημεία. Το `ensemble.py`
προσθέτει ένα **distribution-aware** μέτρο: το πραγματικό ensemble του GEFS, από τα
προϋπολογισμένα πεδία `geavg` (μέσος όρος) και `gespr` (διασπορά). Το NOMADS grib
filter τα περιορίζει server-side στο ελληνικό πλαίσιο, οπότε ένα βήμα κοστίζει λίγα
KB αντί εκατοντάδες MB για τα 30 αρχεία μελών.

Το panel είναι δικό του τμήμα στο PRO (`Σύνολο GEFS`), όχι υποσημείωση κάτω από τη
συμφωνία μοντέλων, γιατί απαντούν σε διαφορετική ερώτηση: η συμφωνία μετρά πόσο
απέχουν τα ντετερμινιστικά μοντέλα μεταξύ τους· το ensemble πόσο απέχουν 30
διαταραγμένες εκδοχές του *ίδιου* μοντέλου από τον μέσο όρο τους.

**Τι περιέχει** (μία γραμμή ανά βήμα: +24, +72, +120, +168, +240 h — όλα εντός του
PRO ορίου των 240 h):

| Μεταβλητή | Πεδίο GEFS | Είδος βήματος |
|---|---|---|
| Θερμοκρασία 2 m | `pgrb2ap5` (0.5°) | στιγμιαίο |
| Άνεμος 10 m (U/V) | `pgrb2ap5` | στιγμιαίο |
| Υετός | `pgrb2ap5` | **συσσώρευση 6 h** |
| Νέφωση | `pgrb2ap5` | **μέσος όρος 6 h** |
| **Ριπές** | **`pgrb2sp25` (0.25°)** | στιγμιαίο |

Οι ριπές **δεν υπάρχουν καθόλου στο 0.5° προϊόν**, οπότε έρχονται με ξεχωριστό,
μικρό (≈1.6 kB) αίτημα στο 0.25° filter. Τα βήματα είναι όλα πολλαπλάσια του 6,
ώστε το παράθυρο συσσώρευσης του υετού να είναι το ίδιο σε κάθε γραμμή και να μην
αθροίζονται ποτέ δύο βήματα σιωπηλά μαζί.

**Τι είναι ακριβώς** (επαληθευμένο από τα `.idx` και τα GRIB metadata των live
προϊόντων, όχι υπόθεση):

- Το `gespr` είναι η διασπορά του ensemble, ορισμού «παρόμοια με τυπική απόκλιση»,
  στο ίδιο unit με το πεδίο. Το UI το λέει «τυπική απόκλιση («ens std dev» κατά NCEP)».
- Τα `geavg`/`gespr` φέρουν `GRIB_dataType="pf"` και `GRIB_totalNumber=30` και
  κατά την NCEP παράγονται **μόνο από τα διαταραγμένα μέλη** — δηλαδή 30 μέλη, όχι
  31. Αυτό είναι το πλήθος που εμφανίζεται.
- Η διασπορά ανέμου είναι το μέτρο του **διανυσματικού** σφάλματος
  `hypot(spread_u, spread_v)`, όχι διασπορά ταχύτητας· το UI το διευκρινίζει.
- Το GRIB **δεν εκθέτει** το παράθυρο συσσώρευσης· το 6-ωρο του υετού προκύπτει από
  το `.idx` και έχει ελεγχθεί σε όλα τα βήματα που χρησιμοποιούνται.

**Τι δεν είναι.** Δεν είναι πιθανότητα επαλήθευσης ούτε «αξιοπιστία». Δεν
επιστρέφεται ποσοστό, ούτε ετικέτα «confidence», ούτε βαθμός high/moderate/low:
μια διασπορά τύπου τυπικής απόκλισης **δεν συγκρίνεται** με την απόκλιση των δύο
μοντέλων (μέγιστη μείον ελάχιστη), οπότε το να περάσει από τα ίδια κατώφλια θα
ήταν κατασκευασμένη σύγκριση. Επίσης δεν παρουσιάζονται percentiles ή πιθανότητες
ανά μέλος: δεν προκύπτουν από mean+spread, και θα παρουσίαζαν στενότερο μέγεθος
ως πλήρη κατανομή.

Είναι **προσθετικό και best-effort**: αν το NOMADS δεν απαντήσει, το panel γίνεται
«μη διαθέσιμο» και η συμφωνία των μοντέλων παραμένει ως έχει. Κάθε μεταβλητή
υποβαθμίζεται ανεξάρτητα — μια αποτυχία στις ριπές δεν αφαιρεί θερμοκρασία, άνεμο,
υετό ή νέφωση. Είναι PRO-only (ζει στο `expert` block του `/api/brief`)· ο FREE δεν
το κατεβάζει και δεν το cache-άρει καθόλου. Το cache key είναι
provider/kind/run/step — ποτέ συντεταγμένες — ώστε όλοι οι PRO χρήστες μιας περιοχής
να μοιράζονται ένα download ανά κύκλο, με single-flight για ταυτόχρονα cold requests
και stale fallback μόνο όταν το φρέσκο fetch έχει ήδη αποτύχει.
